...
name=sys.argv[1]
fold=sys.argv[2]
json_fold=sys.argv[3]
pdf_info_dir=sys.argv[4]
hocr_dir=sys.argv[5]
...
soup=BeautifulSoup(open(fold+name,encoding='utf-8',errors='ignore'), "lxml")
json_output=open(json_fold+"metadata_"+name.rstrip(".xml")+".json",'w+',encoding='utf-8',errors='ignore')
...
def alter():
    try:
        url=hocr_dir+name.strip(".xml")+"/"+name.strip(".xml")+"_0"+".hocr"
        with open(url, 'r',encoding='utf-8',errors='ignore') as myfile:
            data=myfile.read()
...
def alter_copyright():
    try:
        ...
        id=''
        while(num<5):
            url=hocr_dir+name.strip(".xml")+"/"+name.strip(".xml")+"_"+str(num)+".hocr"
            with open(url, 'r',encoding='utf-8',errors='ignore') as myfile:
            	data=myfile.read()
            texts=[]
            word_text=""
            ...
            if len(c_areas)>0:    
            	for i in range(len(c_areas)):
            		...
            		X_text+="\n\n\n"+p_text
            		X_text=X_text.strip("\n\n\n")
            id=re.compile('[(copyright)©(ALL RIGHTS RESERVED)]+.*\d{4}',re.DOTALL|re.IGNORECASE).findall(X_text)
            if(len(id)>0):
            	id=id[0]
            	break
            else:
            	num=num+1 		
        copyright=re.findall(r'\d+',id)
        if len(copyright)>0:
...
if True:
    ...
    for i in re.sub("part","",re.sub("book","",xxx.lower())).split():
    ...
    if re.sub("[(Book)(book)(part)(one)(two)(three)(four)(five)(six)(seven)(eight)\d\sIVX]+","",xxx,re.IGNORECASE)=="" or check==False:
        yyy=alter()[0]
        xxx=""
        for i in yyy.split():
...
#PDF INFO PART
pdfinfo=open(pdf_info_dir+name.rstrip(".xml")+".txt", 'r',encoding='utf-8',errors='ignore').read()
...
level="easy"
if roman.fromRoman(Class)>7 and roman.fromRoman(Class)<13:
    level="medium"
    metadata["lrmi.educationalAlignment.difficultyLevel[]"]=level
else:
    metadata["lrmi.educationalAlignment.difficultyLevel[]"]=""
level="primary"
if roman.fromRoman(Class)>4 and roman.fromRoman(Class)<13:
    level="high school"
    metadata["lrmi.educationalAlignment.educationLevel"]=level
else:
    metadata["lrmi.educationalAlignment.educationLevel"]=""
...
json_output.write(json.dumps(metadata,ensure_ascii=False,indent=4))
